...
if [ !  $1 ] ;  then 
    ...
    if [ $( free -g | awk ' /^Mem:/{print $2} ' )  -lt 3 ] ;  then 
        echo You need 4 GB of RAM to compile MAME with multithreaded support.
        exit
    fi
    ...
    echo Installing/updating to MAME $MAMEVER ...
    fs_unlock
    ...
    if [ !  -x  $MAMEBINPATH /mame] ;  then          # We build only if not already built 
        ...
        BUILDSTART= $( date + %s )
         ...
         echo-------------------------------------------------- ---------------------------------
         echo Please wait  until the build is completed \( about 5 hours \) .. .echo ------------------------------------------------ _
         -----------------------------------
        ...
        if [ -x  $MAMEBINPATH /mame ] ;  then 
            ...
            if [ !  -L /home/pi/settings ] && [[ $( findmnt -n /data ) ]] ;  then     # Symlink not found and /data is mounted... 
              [ !  -f /data/.sys/env/settings ] && mkdir /data/.sys/env
